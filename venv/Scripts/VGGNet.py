...
class VGGNet():
    def __init__(self, root, device, dataset, batch_size, train_size, test_size):
        self.root = root
        self.device = device
        self.model = models.vgg16(pretrained=False, num_classes=24)
        ...
        self.file = os.path.join(root,'VGG16', 'INFO.txt')
        self.FIX_STEP = False
    ...
    def train(self, num_epochs, loss_fn, optimizer,fix_step=False):
        ...
        for epoch in range(num_epochs):
            ...
            model_scripted.save(os.path.join(self.root,'VGG16', 'VGG16_' +str(epoch) + '_' + str(self.batch_size) + '.pt'))
